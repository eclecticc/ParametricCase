...
module traditional(show_body, show_lid, show_internals, heatsink_type, psu_type) {
    ...
    case_origin = [motherboard_back_edge-wall, -zotac_1080_thickness-wall+microatx_pci_e_offset[1]+3, -microatx_bottom_keepout-wall]; // TODO: Clean up the Y calculation
    ...
    microatx_cooling_length = -motherboard_back_edge+wall*3+microatx[0]+(heatsink_type == "aio" ? corsair_h60_size[0] : case_fan_thickness);
    gpu_length = zotac_1080_mini_length+wall*3; // Note the extra wall length for assembly margin

    case_size = [max(microatx_cooling_length, gpu_length), microatx[1]-case_origin[1]+motherboard_back_panel_overhang+motherboard_back_panel_lip, max(psu_heatsink_stack, gpu_stack)];
